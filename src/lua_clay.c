...
clay_lua_build_dimensions(lua_State *L, Clay_Dimensions *data)
{
  ...
  lua_pushvalue(L, data->width);
  lua_setfield(L, dim, "width");
  lua_pushvalue(L, data->height);
...
clay_lua_build_image_command(lua_State *L, int cmd, Clay_ImageRenderData *data)
{
  if (data->imageData)
  {
    lua_rawgeti(L, LUA_REGISTRYINDEX, data->imageData);
...
clay_lua_build_custom_command(lua_State *L, int cmd, Clay_CustomRenderData *data)
{
  ...
  clay_lua_build_cornerRadius(L, &(data->cornerRadius));
  lua_setfield(L, cmd, "cornerRadius");
  lua_rawgeti(L, LUA_REGISTRYINDEX, data->customData);
...
clay_lua_build_render_command(lua_State *L, Clay_RenderCommand *data)
{
  ...
  if (data->userData)
  {
    lua_rawgeti(L, LUA_REGISTRYINDEX, data->userData);
...
clay_lua_build_element_border_width(lua_State *L, int idx, Clay_BorderWidth *width)
{
  ...
  {
    width->bottom = width->left = width->right = width->top = lua_tonumber(L, idx);
  }
  if (!lua_istable(L, idx)) return;
...
clay_lua_build_element_cornerRadius(lua_State *L, int idx, Clay_CornerRadius *corner)
{
  ...
  if (lua_isnumber(L, -1))
  {
    corner->bottomLeft = lua_tonumber(L, -1);
  ...
  if (lua_isnumber(L, -1)) corner->bottomLeft = lua_tonumber(L, -1);
  lua_getfield(L, t, "bottomRight");
  if (lua_isnumber(L, -1)) corner->bottomRight = lua_tonumber(L, -1);
  lua_getfield(L, t, "topLeft");
  if (lua_isnumber(L, -1)) corner->topLeft = lua_tonumber(L, -1);
  lua_getfield(L, t, "topRight");
  if (lua_isnumber(L, -1)) corner->topRight = lua_tonumber(L, -1);
}
...
clay_lua_build_element_custom(lua_State *L, int idx, Clay_CustomElementConfig *custom)
{
  ...
  custom->customData = luaL_ref(L, lua_gettop(L));
}
...
clay_lua_build_element_dimensions(lua_State *L, int idx, Clay_Dimensions *dim)
{
  ...
  if (lua_isnumber(L, -1)) dim->width = lua_tonumber(L, -1);
  lua_getfield(L, idx, "height");
  if (lua_isnumber(L, -1)) dim->height = lua_tonumber(L, -1);
}
...
clay_lua_build_element_parentId(lua_State *L, int idx, uint32_t *parentId)
{
  ...
  Clay_String key = (Clay_String){len, clay_lua_storeString(str, len)};
...
clay_lua_build_element_floating(lua_State *L, int idx, Clay_FloatingElementConfig *floating)
{
  ...
  if (lua_isnumber(L, -1))
  {
    floating->pointerCaptureMode = lua_tonumber(L, -1);
  }
  ...
  if (lua_isnumber(L, -1)) floating->zIndex = lua_tonumber(L, -1);
}
...
clay_lua_build_alignment(lua_State *L, int idx, Clay_ChildAlignment *align)
{
  ...
  if (lua_isnumber(L, idx))
  {
    align->x = align->y = lua_tonumber(L, idx);
    return;
  }
...
clay_lua_build_layout_direction(lua_State *L, int idx, Clay_LayoutDirection *direction)
{
  ...
  if (lua_isnumber(L, idx))
  {
    *direction = lua_tonumber(L, idx);
  }
  else if (lua_isstring(L, idx))
  {
    const char *d = lua_tostring(L, idx);
    if (strcmp(d, "row") == 0 || strcmp(d, "left_to_right") == 0)
    {
      *direction = CLAY_LEFT_TO_RIGHT;
    }
    else if (strcmp(d, "column") == 0 || strcmp(d, "top_to_bottom") == 0)
    {
...
clay_lua_build_padding(lua_State *L, int idx, Clay_Padding *padding)
{
  ...
  if (lua_isnumber(L, idx))
  {
    padding->top = padding->bottom = padding->left = padding->right = lua_tonumber(L, idx);
  }
  else if (lua_istable(L, idx))
  {
    lua_getfield(L, idx, "top");
    if (lua_isnumber(L, -1)) padding->top = lua_tonumber(L, -1);
    lua_getfield(L, idx, "bottom");
    if (lua_isnumber(L, -1)) padding->bottom = lua_tonumber(L, -1);
    lua_getfield(L, idx, "left");
    if (lua_isnumber(L, -1)) padding->left = lua_tonumber(L, -1);
    lua_getfield(L, idx, "right");
    if (lua_isnumber(L, -1)) padding->right = lua_tonumber(L, -1);
  }
}
...
clay_lua_build_sizing_axis(lua_State *L, int idx, Clay_SizingAxis *axis)
{
  ...
  else if (lua_istable(L, idx))
  {
    ...
    if (lua_isnumber(L, -1))
    {
      axis->size.minMax.min = axis->size.minMax.max = lua_tonumber(L, -1);
    }
    lua_getfield(L, idx, "min");
    if (lua_isnumber(L, -1)) axis->size.minMax.min = lua_tonumber(L, -1);
    lua_getfield(L, idx, "max");
    if (lua_isnumber(L, -1)) axis->size.minMax.max = lua_tonumber(L, -1);
...
clay_lua_build_element_layout(lua_State *L, int idx, Clay_LayoutConfig *data)
{
  ...
  if (lua_isnumber(L, -1))
  {
    data->childGap = lua_tonumber(L, -1);
  }
...
{
  Clay_SetMaxElementCount(lua_tonumber(L, 1));
  return 0;
}
...
{
  Clay_SetMaxMeasureTextCacheWordCount(lua_tonumber(L, 1));
  return 0;
}
...
{
  struct HoverData *data = userData;
  lua_State *L = data->L;
  int ref = data->ref;
...
l_onHover(lua_State *L)
{
  ...
  Clay_OnHover(clay_lua_onHover, data);
  return 0;
}
...
clay_lua_build_element_textConfig(lua_State *L, int idx, Clay_TextElementConfig *config)
{
  ...
  if (!lua_isnil(L, -1)) config->fontId = lua_tonumber(L, -1);
  lua_getfield(L, idx, "fontSize");
  if (!lua_isnil(L, -1)) config->fontSize = lua_tonumber(L, -1);
  lua_getfield(L, idx, "hashStringContents");
  config->hashStringContents = lua_toboolean(L, -1);
  lua_getfield(L, idx, "letterSpacing");
  if (!lua_isnil(L, -1)) config->letterSpacing = lua_tonumber(L, -1);
  lua_getfield(L, idx, "lineHeight");
  if (!lua_isnil(L, -1)) config->lineHeight = lua_tonumber(L, -1);
...
l_text(lua_State *L)
{
  size_t len;
  const char *str = lua_tolstring(L, 1, &len);
  Clay_String text = (Clay_String){len, clay_lua_storeString(str, len)};
  Clay_TextElementConfig *config = lua_newuserdata(L, sizeof(Clay_TextElementConfig));
...
clay_lua_measureText(Clay_StringSlice str, Clay_TextElementConfig *config, uintptr_t _)
{
...
l_setMeasureTextFunction(lua_State *L)
{
  ...
  Clay_SetMeasureTextFunction(clay_lua_measureText, NULL);
  return 0;
}
